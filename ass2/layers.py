...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    Input:
    - x: Data of shape (N, D)
    - gamma: Scale parameter of shape (D,) #缩放
    - beta: Shift paremeter of shape (D,) #平移
    ...
    if mode == 'train':
        ...
        mu = (1.0 / float(N)) * np.sum(x, axis=0) #(D,) avg_x
        x_minus_mu =  x - mu #(N, D) x - avg_x
        x_minus_mu_square = (x_minus_mu) ** 2 #(N, D) (x - avg_x)^2
        var = ((1.0 / float(N))) * np.sum(x_minus_mu_square,axis=0) #(D,) (1/n)*(x - avg_x)^2 方差
        sqrt_var = 1.0 * np.sqrt(var+eps) #(D,) ^(1/2) 标准差
        sqrt_var_invert = 1.0 / sqrt_var #(D,) std 标准差的倒数
        x_norm = x_minus_mu / sqrt_var #(N, D) 归一化

        cache = (N,mu,x_minus_mu,x_minus_mu_square,var,sqrt_var,sqrt_var_invert,x_norm,eps,gamma)
        pass
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    N,mu,x_minus_mu,x_minus_mu_square,var,sqrt_var,sqrt_var_invert,x_norm,eps,gamma = cache
    #求导，主要思想就是将复杂的公式分解为 单位个体 求解, 在中间过程调整维度
    dgamma = np.sum(x_norm*dout,axis=0) #(1, D) / (D, )
    dbeta = np.sum(dout,axis=0) #(1, D) / (D, )

    dx_norm = gamma * dout # (D,) * (N, D) = (N, D)

    dx_minus_mu = dx_norm * sqrt_var_invert # (N, D)
    dsqrt_var_invert = np.sum(dx_norm * x_minus_mu, axis=0) # (D, )

    dsqrt_var = dsqrt_var_invert * (-1.0) / (sqrt_var**2) #(D, )

    dvar = dsqrt_var * (0.5) * ((var+eps)**(-0.5)) #(D, )

    dx_minus_mu_square =  (1.0/float(N)) * np.ones(x_minus_mu_square.shape) * dvar #(N, D)

    dx_minus_mu += dx_minus_mu_square * 2.0 * x_minus_mu#(N, D)
    dx = dx_minus_mu#(N, D)

    dmu = -1.0 * np.sum(dx_minus_mu,axis=0)#(D)
    dx += (1.0/float(N)) * np.ones(dx_minus_mu.shape) * dmu#(N, D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    # grad = dout * gamma * eps * (((var + eps)**(-1.5)) * np.ones(x_minus_mu.shape)) #(N, D)
    #
    # dx = grad  # (N, D)
    # dmu = -1.0 * np.sum(grad, axis=0)  # (D)
    #
    # dx += (1.0 / float(N)) * np.ones(x_minus_mu.shape) * dmu  # (N, D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    #并没有看懂，可能是维度处理的问题
    o = dout*gamma
    dx = (1.0/N) * sqrt_var_invert*(N*o-np.sum(o*x_norm,axis=0,keepdims=True)*x_norm-np.sum(o,axis=0,keepdims=True))
...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(x_norm.T * dout, axis=0)  # (1, D) / (D, )
    dbeta = np.sum(dout, axis=0)  # (1, D) / (D, )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    # 并没有看懂，可能是维度处理的问题
    o = dout.T * gamma.reshape(gamma.shape[0],-1)
    dx = (1.0 / N) * sqrt_var_invert * (
                N * o - np.sum(o * x_norm, axis=0, keepdims=True) * x_norm - np.sum(o, axis=0, keepdims=True))
    dx = dx.T
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
